...
import { asyncremoveemploye, asynccurrentemploye, asyncinternshipcreate } from '../../store/Actions/userActions'
...
const Internship = () => {
    ...
    const validateInput = (name, value) => {
        switch (name) {
            ...
            case 'skill':
                // You can add any specific validation for the skill field if needed
                return '';
            case 'internshiptype':
                return value.trim() === '' ? 'Internship type is required' : '';
            case 'city':
            ...
            case 'internshipDuration':
                return value.trim() === 'select' ? 'Please select internship duration' : '';
            case 'responsibilities':
                return value.trim() === '' ? 'Responsibilities are required' : '';
            case 'status':
                return value.trim() === '' ? 'Stipend status is required' : '';
            case 'amount':
                return isNaN(value) || value < 0 ? 'Invalid stipend amount' : '';
            case 'time':
    ...
    if (Object.values(newErrors).every((error) => error === '')) {
        ...
        // Dispatch the asynchronous action and wait for it to complete
         dispatch(asyncinternshipcreate(formData));
        // After the action is complete, navigate to the new route
        // navigate("/JobInternship");
    } else {
        // await dispatch(asyncinternshipcreate(formData));
    ...
    return employe ? (
        <div>
            ...
            <div className='w-full min-h-[320vh] mt-5'>
                <p className='text-center uppercase text-blue-400 text-3xl mb-1'>Internship Details</p>
                <div className='m-auto h-[2px] w-[23vw] bg-blue-500'></div>
                <div className='w-[50%] m-auto min-h-[320vh]'>
                    <form onSubmit={handleSubmit}>
                        <div className='w-[100%] h-[210vh] border-2 mt-14 rounded-xl p-8'>
                            <p className='font-semibold text-zinc-600'>Internship profile </p>
                            <input type="text" className='w-full border mt-3 mb-2 h-9 rounded-sm px-2' name='profile' value={formData.profile} onChange={handleChange} />
                            ...
                            <input type="text" className='w-full border mt-3 mb-2 h-9 rounded-sm px-2' name='skill' value={formData.skill} onChange={handleChange} />
                            ...
                            <div>
                                <p className='font-semibold text-zinc-600'>Internship type </p>
                                <div className='flex gap-5 mb-3'>
                                    <div className='flex items-center gap-3'>
                                        <input type="radio" name='internshiptype' value="In office" className='w-5 border mt-3 mb-2 h-5 rounded-sm' onChange={handleChange} />
                                        <p className='text-zinc-600'>In office</p>
                                    </div>
                                    <div className='flex items-center gap-3'>
                                        <input type="radio" name='internshiptype' value="Hybrid" className='w-5 border mt-3 mb-2 h-5 rounded-sm' onChange={handleChange} />
                                        <p className='text-zinc-600'>Hybrid</p>
                                    </div>
                                    <div className='flex items-center gap-3'>
                                        <input type="radio" name='internshiptype' value="Remote" className='w-5 border mt-3 mb-2 h-5 rounded-sm' onChange={handleChange} />
                            ...
                            <input type="text" className='w-full border mt-3 mb-2 h-9 rounded-sm px-2' name='city' value={formData.city} onChange={handleChange} />
                            <p className='font-semibold text-zinc-600'>Number of openings </p>
                            <input type="text" className='w-full border mt-3 mb-2 h-9 rounded-sm px-2' name='openings' value={formData.openings} onChange={handleChange} />

                            <div className='flex items-center justify-between mb-2'>
                                <div className='flex flex-col'>
                                    <p className='font-semibold text-zinc-600'>Internship Start date </p>
                                    <input type="date" className='w-full border mt-3 mb-2 h-9 rounded-sm' name='internshipStartDate' value={formData.internshipStartDate} onChange={handleChange} />
                                </div>
                                <div className='flex flex-col'>
                                    <p className='font-semibold text-zinc-600'>Internship End date</p>
                                    <input type="date" className='w-full border mt-3 mb-2 h-9 rounded-sm' name='internshipEndDate' value={formData.internshipEndDate} onChange={handleChange} />
                                </div>
                            </div>
                            <div class="mb-4">
                                <p className='font-semibold text-zinc-600 mb-1'>Internship duration</p>
                                <p className=' text-zinc-600 mb-3 text-sm'>Shorter the duration, more the applications</p>
                                <select id="department" name="internshipDuration" class="w-full p-2 border border-gray-300 rounded" value={formData.internshipDuration} onChange={handleChange}>
                                    <option  name="internshipDuration">select</option>
                                    <option value="1 Month" name="internshipDuration">1 Month</option>
                                    <option value="2 Month" name="internshipDuration">2 Month</option>
                                    <option value="3 Month" name="internshipDuration">3 Month</option>
                                    <option value="4 Month" name="internshipDuration">4 Month</option>
                                    <option value="5 Month" name="internshipDuration">5 Month</option>
                                    <option value="6 Month" name="internshipDuration">6 Month</option>
                                </select>
                            </div>
                            <p className='font-semibold text-zinc-600 mb-3'>Intern’s responsibilities</p>
                            <div className='w-full h-48 overflow-hidden border-2 rounded-lg'>
                                <textarea className='w-full h-[100%] p-2 overflow-scroll resize-none bg-zinc-100' name='responsibilities' value={formData.responsibilities} onChange={handleChange}>
                        ...
                        <p className='font-semibold text-zinc-600 mb-3 mt-10 text-[18px]'>Stipend & perks</p>
                        <div className='w-[100%] h-[57vh] border-2 mt-1 rounded-xl p-8'>
                            <p className='font-semibold text-zinc-600'>Stipend </p>
                            <div className='flex gap-5 mb-3'>
                                <div className='flex items-center gap-3'>
                                    <input type="radio" name='status' className='w-5 border mt-3 mb-2 h-5 rounded-sm' value="Fixed" onChange={handleChange}/>
                                    <p className='text-zinc-600'>Fixed</p>
                                </div>
                                <div className='flex items-center gap-3'>
                                    <input type="radio" name='status' className='w-5 border mt-3 mb-2 h-5 rounded-sm' value='Negotiable' onChange={handleChange}/>
                                    <p className='text-zinc-600'>Negotiable</p>
                                </div>
                                <div className='flex items-center gap-3'>       
                                    <input type="radio" name='status' className='w-5 border mt-3 mb-2 h-5 rounded-sm' value='Performance based' onChange={handleChange}/>
                                    <p className='text-zinc-600'>Performance based</p>
                                </div>
                                <div className='flex items-center gap-3'>
                                    <input type="radio" name='status' className='w-5 border mt-3 mb-2 h-5 rounded-sm' value='Unpaid' onChange={handleChange}/>
                                    <p className='text-zinc-600'>Unpaid</p>
                                </div>
                            </div>
                            <div>
                                <input className='w-full border mt- mb-2 h-9 rounded-sm px-3' type="text" placeholder='Enter the Salary..' name='amount' value={formData.amount} onChange={handleChange} />
                            </div>
                        ...
                        <div className='w-[100%] h-[53vh] border-2 mt-1 rounded-xl p-8'>
